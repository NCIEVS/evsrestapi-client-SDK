...
#
if [ $# -ne 3 ]; then
  echo "Usage: $0 <terminology> <list> <include>"
  exit 1
fi
terminology=$1
list=$2
...
if [ $status -ne 200 ]; then
  ...
  echo "ERROR: GET $url/concept/$terminology?list=$list returned $status, expected 200"
  exit 1
fi
